...
import { getViewerDetails, isLocalFile, replaceLocalUrl, iframeIsLoaded, getDocxToHtml, googleCheckSubscription } from 'docviewhelper';
...
const DocumentViewer = inputProps => {
  ...
  const props = useRef();
  const setNewurl = details => __awaiter(void 0, void 0, void 0, function* () {
    if (props.current) {
      const iframe = iframeRef.current;
      if (checkIFrameSubscription && checkIFrameSubscription.current) {
        checkIFrameSubscription.current.unsubscribe();
      }
      if (props.current.viewer === 'google' && props.current.googleCheckContentLoaded === true) {
        reloadIframe(iframe, details.url, props.current.googleCheckInterval, props.current.googleMaxChecks);
      }
    }
  });
  useEffect(() => {
    ...
    if (state.isSwitching) {
      ...
      timerRef = setTimeout(() => {
        setNewurl({
          url: state.url,
          externalViewer: state.externalViewer
        });
      }, 500);
    }
    return () => {
  ...
  useEffect(() => {
    props.current = Object.assign(Object.assign({}, defaultProps), inputProps);
    let details = getViewerDetails(props.current.url, props.current.viewer, props.current.queryParams, props.current.viewerUrl);
    if (details.externalViewer && props.current.overrideLocalhost && isLocalFile(props.current.url)) {
      const newUrl = replaceLocalUrl(props.current.url, props.current.overrideLocalhost);
      details = getViewerDetails(newUrl, props.current.viewer, props.current.queryParams, props.current.viewerUrl);
    }
    setState({
      url: details.url,
    ...
    if (iframeRef && iframeRef.current) {
      ...
      const setHtml = () => __awaiter(void 0, void 0, void 0, function* () {
        const docHtml = {
          __html: yield getDocxToHtml(details.url)
        };
        setState({
          url: '',
          docHtml,
  ...
  const reloadIframe = (iframe, url, interval, maxChecks) => {
  ...
  const iframeLoaded = () => {
    if (props.current && iframeRef && iframeRef.current !== null && iframeIsLoaded(iframeRef.current)) {
  ...
  return state.isSwitching ? null : state.externalViewer ? jsx("iframe", {
    style: (_a = props.current) === null || _a === void 0 ? void 0 : _a.style,
    className: (_b = props.current) === null || _b === void 0 ? void 0 : _b.className,
    ref: iframeRef,
    onLoad: () => {
      iframeLoaded();
    },
    ...
    frameBorder: "0",
    src: state.url
  }) : ((_c = props.current) === null || _c === void 0 ? void 0 : _c.viewer) !== 'pdf' ? jsx("div", {
    dangerouslySetInnerHTML: state.docHtml
  }) : state.url ? jsx("object", Object.assign({
    data: state.url,
    ...
    children: jsxs("p", {
      children: ["Your browser does not support PDFs.", jsx("a", Object.assign({
        href: state.url
      }, {
